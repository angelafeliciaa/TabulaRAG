...
def _make_user(**kwargs) -> User:
    defaults = {"id": 1, "provider": "github", "provider_id": "42",
                "name": "Tester", "email": None, "avatar_url": ""}
...
def test_github_callback_success(client):
    mock_github_user = {"id": 1, "login": "octocat", "name": "Octo", "avatar_url": ""}
    mock_db_user = _make_user(name="Octo", avatar_url="")

    with patch("app.main.exchange_github_code", new_callable=AsyncMock, return_value=mock_github_user), \
         patch("app.main.upsert_user", return_value=mock_db_user):
...
def test_normalize_headers():
    from app.main import _normalize_headers
    result = _normalize_headers(["Name", "", "Name", "  Age  "])
